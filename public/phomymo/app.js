...
function decodeAutoLabelPayload(encoded) {
  if (!encoded) {
    return null;
  }
  ...
  try {
    ...
    console.warn('Failed to decode autolabel payload:', e);
    return null;
  }
}
...
function applyAutoLabelPayload(payload) {
  ...
  lines.forEach((line, idx) => {
    ...
    text.fontSize = idx === 0 ? 13 : 9;
    text.fontWeight = idx === 0 ? 'bold' : 'normal';
    text.noWrap = true;
    text.clipOverflow = true;
    state.elements.push(text);
  });

  const barcodeElement = createBarcodeElement(barcode, {
    x: margin,
  ...
  barcodeElement.showText = false;
  barcodeElement.barcodeFormat = 'CODE128';
